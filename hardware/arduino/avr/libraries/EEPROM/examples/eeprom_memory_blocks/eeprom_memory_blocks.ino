...
void setup() {
  ...
  ***/
  ...
  int *dynamicData = new int[ itemCount ];

  // Fill array with some data and print each element.
  for( int index = 0 ; index < itemCount ;  ++index ){
  
    dynamicData[ index ] = random(0x7FFF);  //The value 0x7FFF is the maximum value for an int on the AVR.
  ...
  EEPROM.put( address, dynamicData, itemCount );
  
  /***
	Now, to show both versions of dynamic memory usage, the written
	data is read back into an allocation provided by malloc().
  ...
  ***/
  ...
  EEPROM.get( address, resultData, itemCount );
